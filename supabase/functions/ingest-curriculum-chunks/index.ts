...
//
// Input:  { upload_id: string, class_id: string, track?: string | null }
// Output: { success: true, upload_id, class_id, track, chunks: number }
//
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  if (!UUID_RE.test(classId)) return fail("`class_id` is required and must be a UUID.", 400);
