-- Coins shown on every social surface (class leaderboard, friends tab, partner
-- directory + snapshot, student search) must equal the InvestiCoin counter in
-- the top bar. That counter is the live jeffs_history ledger balance
-- (SUM of amounts — earnings minus spending).
--
-- The deployed RPCs had drifted to `ledger + portfolio cost basis` (a net-worth
-- style score) with no matching migration in the repo, so the leaderboard /
-- snapshot numbers read HIGHER than the top-bar coins. This migration removes
-- the portfolio term everywhere so all "Coins" numbers agree, and re-syncs the
-- repo with the live database. The returned column keeps its `xp` name so no
-- client code has to change shape.
...
-- Keeps the holdings/business detail blocks; only the top-level `xp` number is
-- normalized to the ledger balance.
  ...
  SELECT CASE WHEN auth.uid() IS NULL THEN NULL ELSE jsonb_build_object(
    ...
    'holdings', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'symbol', pf.symbol, 'shares', pf.shares, 'purchase_price', pf.purchase_price
    ...
    'business', (
      SELECT jsonb_build_object(
        'week', b.state->'week', 'revenue', b.state->'revenue',
        'credit_score', b.state->'creditScore', 'star_rating', b.state->'starRating',
