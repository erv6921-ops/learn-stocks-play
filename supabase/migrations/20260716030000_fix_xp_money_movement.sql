-- Bug fix: "coins earned" (XP) counted every positive jeffs_history entry,
-- including pure money MOVEMENT — stock sale proceeds, InvestiBank vault
-- withdrawals, loan disbursements, bond payouts and challenge refunds. A
-- deposit + withdrawal round-trip doubled as fake XP, so leaderboard numbers
-- ran far ahead of what students actually earned.
--
-- Fix: a shared is_xp_reason() predicate, applied to every server-side XP
-- computation. Must stay in sync with NON_EARNING_PREFIXES in
-- src/lib/playerStats.ts.

CREATE OR REPLACE FUNCTION public.is_xp_reason(_reason text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT NOT (
    _reason ILIKE 'sold %'                      -- stock sale proceeds
    OR _reason ILIKE 'withdrew from investibank%' -- vault withdrawal
    OR _reason ILIKE 'borrowed:%'               -- loan disbursement
    OR _reason ILIKE 'bond matured%'            -- bond principal + interest payout
    OR _reason ILIKE 'challenge refund%'        -- refunded entry fee
  )
$$;
GRANT EXECUTE ON FUNCTION public.is_xp_reason(text) TO authenticated;
  ...
  SELECT
    ...
    COALESCE((
      ...
      WHERE jh.user_id = p.id
        AND jh.amount > 0
        AND public.is_xp_reason(jh.reason)
    ), 0) AS xp
...
AS $$
  SELECT
    ...
    COALESCE((
      ...
      WHERE jh.user_id = p.id AND jh.amount > 0 AND public.is_xp_reason(jh.reason)
    ), 0) AS xp,
    COALESCE((
      SELECT CASE
...
AS $$
  SELECT CASE WHEN auth.uid() IS NULL THEN NULL ELSE jsonb_build_object(
    ...
    'xp', COALESCE((
      SELECT SUM(jh.amount) FROM public.jeffs_history jh
      WHERE jh.user_id = _user_id AND jh.amount > 0 AND public.is_xp_reason(jh.reason)
    ), 0),
    'partner_count', (
