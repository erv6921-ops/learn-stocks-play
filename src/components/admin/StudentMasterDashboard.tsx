...
// Compact "how long ago" label for a student's last activity, e.g. "3h ago",
// "2 days ago", "just now". Returns "Never active" when the RPC gave us no
// timestamp (a student who has never done anything countable).
const timeAgo = (iso: string | null): string => {
  if (!iso) return "Never active"
  const then = new Date(iso).getTime()
  if (!Number.isFinite(then)) return "Never active"
